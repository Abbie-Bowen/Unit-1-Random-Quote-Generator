...
//variables
let randomNumber; //stores random numbers generated
let repeatNumberArray = []; //stores randomNumber returns to check for repetition
let repeats; //stores boolean test of whether the randomNumber is already in the repeatNumberArray
...
function getRandomQuote() {
  randomNumber = Math.floor(Math.random() * quotes.length);//random number variable produces a number between 0 and quotes length
  repeats = (repeatNumberArray.includes(randomNumber)); //returns boolean to test if random number has been inserted in array previously
  ...
  ***/
  if (repeats === false && repeatNumberArray.length < quotes.length-2) {
    ...
    ***/
    } else if (repeats === false && repeatNumberArray.length >= quotes.length-2) {
...
//confirms that season exists in the quote property
    if (randomQuoteObject.season) {
      withSeason = ` Season ${randomQuoteObject.season}`;
    } else {
      withSeason = "";
    }
//confirms that citation exists in the quote property
    if (randomQuoteObject.citation) {
      withCitation =` : "${randomQuoteObject.citation}"`;
    } else {
      withCitation = "";
    }
//confirms that year exists in the quote property
    if (randomQuoteObject.year) {
      withYear =` ${randomQuoteObject.year}.`;
    } else {
      withYear = "";
    }
//concatinates the properties into an htmlString variable
    htmlString = `<p class="quote"> ${randomQuoteObject.quote}</p>
    <p class="source"> ${randomQuoteObject.source} Rose, <span>${withSeason}${withCitation}</span><span>${withYear}</span></p>`;
